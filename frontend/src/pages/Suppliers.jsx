...
export default function Suppliers() {
  ...
  const load = () => {
    setLoading(true)
    getSuppliers().then(r => setSuppliers(r.data)).catch(() => toast.error('خطأ في تحميل البيانات')).finally(() => setLoading(false))
  }

  useEffect(() => { load() }, [])

  const openAdd = () => { setForm(emptyForm); setEditId(null); setShowModal(true) }
  const openEdit = (s) => { setForm({ name: s.name || '', contact_name: s.contact_name || '', email: s.email || '', phone: s.phone || '', address: s.address || '' }); setEditId(s.id); setShowModal(true) }

  const handleSubmit = async (e) => {
    e.preventDefault(); setSaving(true)
    try {
      if (editId) { await updateSupplier(editId, form); toast.success('تم التحديث') }
      else { await createSupplier(form); toast.success('تم الإضافة') }
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteSupplier(id); toast.success('تم الحذف'); load() } catch { toast.error('خطأ في الحذف') }
  }
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {suppliers.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-10 text-gray-400">لا يوجد موردون</td></tr>
              ) : suppliers.map(s => (
                <tr key={s.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button onClick={() => openEdit(s)} className="text-indigo-600 hover:text-indigo-800 p-1"><Pencil size={16} /></button>
                      <button onClick={() => handleDelete(s.id)} className="text-red-500 hover:text-red-700 p-1"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h3 className="text-lg font-bold text-gray-800">{editId ? 'تعديل المورد' : 'إضافة مورد جديد'}</h3>

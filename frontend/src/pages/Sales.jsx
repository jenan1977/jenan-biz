...
import { getInvoices, createInvoice, deleteInvoice } from '../services/invoicesService'
import { getCustomers } from '../services/customersService'
import { getProducts } from '../services/productsService'
import toast from 'react-hot-toast'
import { Plus, Trash2, X, Eye, PlusCircle, MinusCircle } from 'lucide-react'

const statusMap = { draft: 'مسودة', sent: 'مُرسلة', paid: 'مدفوعة', cancelled: 'ملغية' }
const statusColor = { draft: 'bg-gray-100 text-gray-700', sent: 'bg-blue-100 text-blue-700', paid: 'bg-green-100 text-green-700', cancelled: 'bg-red-100 text-red-700' }
...
export default function Sales() {
  ...
  const load = () => {
    setLoading(true)
    Promise.all([getInvoices(), getCustomers(), getProducts()])
      .then(([inv, cust, prod]) => { setInvoices(inv.data); setCustomers(cust.data); setProducts(prod.data) })
  ...
  const costTotal = lines.reduce((s, l) => {
    ...
    return s + (Number(l.quantity) * (prod?.purchase_price ?? 0) || 0)
  }, 0)
  ...
  const handleLineProductChange = (i, pid) => {
    ...
    if (p) updateLine(i, 'unit_price', p.sale_price ?? '')
  }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setShowCreate(false); setCustomerId(''); setLines([{ product_id: '', quantity: 1, unit_price: '' }]); setApplyTax(false); setNotes('')
      load()
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteInvoice(id); toast.success('تم الحذف'); load() } catch { toast.error('خطأ في الحذف') }
  }
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                {['رقم الفاتورة','العميل','التاريخ','الإجمالي','الضريبة','المجموع','الربح','الحالة','إجراءات'].map(h => (
            ...
            <tbody className="divide-y divide-gray-50">
              {invoices.length === 0 ? (
                <tr><td colSpan={9} className="text-center py-10 text-gray-400">لا توجد فواتير</td></tr>
              ) : invoices.map(inv => (
                <tr key={inv.id} className="hover:bg-gray-50 transition">
                  <td className="px-4 py-3 font-medium text-indigo-700">#{inv.invoice_number || inv.id}</td>
                  <td className="px-4 py-3 text-gray-700">{customers.find(c => c.id === inv.customer_id)?.name || inv.customer_id}</td>
                  <td className="px-4 py-3 text-gray-600">{inv.date ? new Date(inv.date).toLocaleDateString('ar-SA') : '-'}</td>
                  <td className="px-4 py-3 text-gray-600">{(inv.subtotal ?? 0).toFixed(2)}</td>
                  <td className="px-4 py-3 text-gray-600">{(inv.tax_amount ?? 0).toFixed(2)}</td>
                  <td className="px-4 py-3 font-semibold text-gray-800">{(inv.total_amount ?? 0).toFixed(2)}</td>
                  <td className="px-4 py-3 text-emerald-600 font-medium">{(inv.profit ?? 0).toFixed(2)}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button onClick={() => setViewItem(inv)} className="text-blue-500 hover:text-blue-700 p-1"><Eye size={16} /></button>
                      <button onClick={() => handleDelete(inv.id)} className="text-red-500 hover:text-red-700 p-1"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h3 className="text-lg font-bold text-gray-800">إنشاء فاتورة مبيعات</h3>
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-5">
              ...
              <div className="flex items-center gap-3">
                <input type="checkbox" id="tax-s" checked={applyTax} onChange={e => setApplyTax(e.target.checked)} className="w-4 h-4 text-indigo-600" />
                <label htmlFor="tax-s" className="text-sm text-gray-700">تطبيق ضريبة القيمة المضافة (15%)</label>
              </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-1 text-sm">
                ...
                <div className="flex justify-between text-emerald-600 font-medium border-t border-gray-100 pt-1"><span>الربح المتوقع</span><span>{profit.toFixed(2)} ر.س</span></div>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-4">
                <div><span className="text-gray-500">العميل: </span><span className="font-medium">{customers.find(c => c.id === viewItem.customer_id)?.name || viewItem.customer_id}</span></div>
              ...
              <div className="bg-gray-50 rounded-xl p-3 space-y-1">
                ...
                <div className="flex justify-between text-emerald-600 font-medium"><span>الربح</span><span>{(viewItem.profit ?? 0).toFixed(2)} ر.س</span></div>
              </div>
